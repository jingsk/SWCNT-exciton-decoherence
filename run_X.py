...
def X_integrand(q,t):
    return gamma(q)**2 *(1+2*n_T(q)) * (1-np.cos(v_s * q *t))

def X0_integrand(q,t):
    return gamma(q)**2 * (1-np.cos(v_s * q *t))

def XT_integrand(q,t):
    return gamma(q)**2 *(2*n_T(q)) * (1-np.cos(v_s * q *t))
...
if __name__ == "__main__":
    ...
    ax.plot(t_array2,norms[0], t_array2,norms[1], t_array2,norms[2])
    #ax.axis('off')
    ax.set_xlabel('Time (ps)')
    plt.setp(ax, yscale='log', ylim=[0.08,1.1],xlim=[-0.01,2.5])
    #ax.xlabel('Time (ps)')
